...
# =============================================================================
# style.css is embedded directly in this script and written to WEBDIR on every
# run — no manual copy needed, works identically for test and production runs.
# app.js is copied from BASE_DIR (it lives alongside these scripts).
...
# --- Copy app.js from BASE_DIR -----------------------------------------------
if [[ -f "${BASE_DIR}/app.js" ]]; then
    cp -p "${BASE_DIR}/app.js" "${WEBDIR}/app.js" && \
        log INFO "app.js staged → ${WEBDIR}/app.js"
else
    log WARN "app.js not found in BASE_DIR: ${BASE_DIR}/app.js"
    log WARN "  Place app.js alongside these scripts in ${BASE_DIR}"
    log WARN "  Dashboard charts and filters will not work until it is present."
fi
