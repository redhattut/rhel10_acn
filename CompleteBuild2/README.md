...
- **Fixed the plaintext root password issue.** Old templates had
  ...
  requires a pre-hashed password via `/etc/completebuild2/rootpw.hash`
  (never in the repo, never plaintext in a `.ks` file).
...
## Credentials — three separate mechanisms, don't conflate them

1. **`xsmrgautomat` AD account** (password-based, via `sshpass`) — used for
   iDRAC and UCSM SSH CLI logins, **and** for GOMP API auth. Different login
   syntax for each:
   ...
   - GOMP: not SSH at all — an `Authorization` header in `gomp.headers`
     (see below), which needs to be regenerated by hand if the password
     ever rotates; it does not update itself from `xsmrgautomat.pw`.
2. **SSH key auth, lmrg34ja → lmrg34ga** — used by `lib/iso_gen.sh` to
   assemble the boot ISO. Whatever account runs `build.sh`/
   `build_server.sh` needs its public key already authorized on `lmrg34ga`.
3. **SSH key auth, lmrg34ja → the newly-built server** — used by
   `lib/post_install.sh` for extra-disk/LVM configuration after the OS is
   up. Same account's public key needs to be in `authorized_keys` below so
   it lands on every new server via kickstart's `%post`.

All secrets live in one directory, outside the code tree:

```
/etc/completebuild2/                    (mode 700, owned by the service account)
├── xsmrgautomat.pw        (400)  plaintext password, mechanism #1 above
├── gomp.headers           (400)  raw header line, e.g. "Authorization: Basic <base64>"
├── rootpw.hash            (400)  openssl passwd -6 output for kickstart's rootpw
└── authorized_keys        (400)  public key(s) for mechanisms #2 and #3
```

`build.sh` and `build_server.sh` both call `check_secrets()` at startup and
fail immediately (before touching any hardware) if `xsmrgautomat.pw` or
`gomp.headers` is missing, or warn if `xsmrgautomat.pw` isn't mode 400.
