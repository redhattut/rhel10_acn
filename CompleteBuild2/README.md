...
- **Fixed the plaintext root password issue.** Old templates had
  ...
  requires a pre-hashed password via `/etc/xsmrgautomat.rootpw.hash` on
  `lmrg34ja` (never in the repo, never plaintext in a `.ks` file).
...
- `/etc/xsmrgautomat.pw` — sshpass password file for the `xsmrgautomat`
  jump account (referenced by `run_racadm`/`run_ucsm` in `common.sh`).
- `/etc/xsmrgautomat.rootpw.hash` — a `openssl passwd -6` (or equivalent)
  hash for the root password kickstart will set.
- `/etc/xsmrgautomat.authorized_keys` — the SSH keys that should land in
  each new server's `~root/.ssh/authorized_keys` (previously hardcoded
  inline in `templateDELL.ks`/`templateCISCO.ks`).
