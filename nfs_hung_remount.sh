#!/bin/bash
# =============================================================================
# nfs_hung_remount.sh — Detect and recover hung NFS mounts after a network outage
# =============================================================================
#
# BACKGROUND:
...
#
# USAGE:
#   nfs_hung_remount.sh --hosts /path/to/hosts.txt [OPTIONS]
#
# OPTIONS:
...
#   nfs_hung_remount.YYMMDDHHNN            Full timestamped log of this run
#   nfs_hung_remount_hung.YYMMDDHHNN       Detail of every hung/failed mount
#   nfs_hung_remount_hosts.YYMMDDHHNN      Deduplicated host list — ready to pass back to --hosts
#
# EXAMPLES:
#   Dry run against weekend change list — see what's hung, touch nothing:
#     ./nfs_hung_remount.sh --hosts /tmp/weekend_servers.txt --dry-run
#
#   Live recovery run:
#     ./nfs_hung_remount.sh --hosts /tmp/weekend_servers.txt
#
#   Tighter stat timeout for fast networks:
#     ./nfs_hung_remount.sh --hosts /tmp/weekend_servers.txt --timeout 3
#
#   After recovery, re-run dry-run to confirm nothing is still hung:
#     ./nfs_hung_remount.sh --hosts /tmp/weekend_servers.txt --dry-run
#
#   Re-run against only the servers that are still hung (hosts file from prior run):
#     ./nfs_hung_remount.sh --hosts /opt/mrgtools/nfs_hung_remount_hosts.YYMMDDHHNN
#
# =============================================================================
...
EXAMPLES:
  ...
  # Re-run against only servers still hung (use generated hosts file)
  ${SCRIPT_NAME} --hosts ${LOG_DIR}/nfs_hung_remount_hosts.YYMMDDHHNN
...
# Log setup
# =============================================================================
# All files are written to the same directory as the script.
# No subdirectory needed — drop the script anywhere and run it.
#
#   nfs_hung_remount.YYMMDDHHNN        Full run log
#   nfs_hung_remount_dryrun.YYMMDDHHNN Full run log (dry-run)
#   nfs_hung_remount_hung.YYMMDDHHNN   Per-mount detail of everything still hung
#   nfs_hung_remount_hosts.YYMMDDHHNN  Deduplicated host list of still-hung servers

if ${DRY_RUN}; then
    LOG_FILE="${LOG_DIR}/nfs_hung_remount_dryrun.${RUN_STAMP}"
else
    LOG_FILE="${LOG_DIR}/nfs_hung_remount.${RUN_STAMP}"
fi
HUNG_LOG="${LOG_DIR}/nfs_hung_remount_hung.${RUN_STAMP}"
HUNG_HOSTS="${LOG_DIR}/nfs_hung_remount_hosts.${RUN_STAMP}"

: > "${LOG_FILE}"
: > "${HUNG_LOG}"
: > "${HUNG_HOSTS}"

# Write hung log header — structured columns, human-readable and greppable
{
    printf "# Generated by %s — %s\n" "${SCRIPT_NAME}" "$(date '+%Y-%m-%d %H:%M:%S')"
    if ${DRY_RUN}; then
        printf "# Servers with hung mounts detected (dry-run — no recovery attempted)\n"
    else
        printf "# Servers with mounts still hung after recovery attempt\n"
    fi
    printf "# To recover: ./%s --hosts %s\n" "${SCRIPT_NAME}" "${HUNG_HOSTS}"
    printf "#\n"
    printf "# %-30s %-35s %s\n" "HOST" "MOUNTPOINT" "METHOD"
    printf "# %-30s %-35s %s\n" "------------------------------" \
        "-----------------------------------" "-------"
} >> "${HUNG_LOG}"
...
log_hung()      { _log_file "HUNG"      "$1"; }
log_dryrun()    { _log_file "DRY_RUN"   "$1"; }
log_recovered() { _log_file "RECOVERED" "$1"; }
log_failed()    { _log_file "FAILED"    "$1"; }
log_skipped()   { _log_file "SKIPPED"   "$1"; }
log_warn()      { _log_file "WARN"      "$1"; }
log_summary()   { _log_file "SUMMARY"   "$1"; }

# Writes a structured row to the hung log and adds the host to the hosts file
# (deduplicated — grep prevents duplicate entries)
_log_hung_entry() {
    local host="$1" mountpoint="$2" method="$3"
    printf "  %-30s %-35s %s\n" "${host}" "${mountpoint}" "${method}" >> "${HUNG_LOG}"
    grep -qxF "${host}" "${HUNG_HOSTS}" 2>/dev/null || echo "${host}" >> "${HUNG_HOSTS}"
}
...
PSSH_TMP=$(mktemp -d /var/tmp/nfs_hung_remount.XXXXXX)
...
# Filter out NetApp .snapshot paths — these are auto-exposed snapshot
# directories under the real NFS mount, not mountpoints we manage.
# Example: /data_security/.snapshot/daily.2026-05-22_0010 — ignore entirely.
grep -v '/.snapshot/' "\$TMPFILE" > "\${TMPFILE}.filtered" \
    && mv "\${TMPFILE}.filtered" "\$TMPFILE" || true
...
# Count unique still-hung hosts from the hosts file
CNT_HUNG_HOSTS=0
[[ -s "${HUNG_HOSTS}" ]] && CNT_HUNG_HOSTS=$(wc -l < "${HUNG_HOSTS}")
...
if ${DRY_RUN}; then
    if [[ ${CNT_HUNG} -eq 0 ]]; then
        ...
        printf "  %-28s %s\n" "Hung mounts detected:"   "${CNT_HUNG}"
        printf "  %-28s %s\n" "Servers affected:"       "${CNT_HUNG_HOSTS}"
        echo
    ...
    [[ ${CNT_SKIPPED} -gt 0 ]] && \
        printf "  %-28s %s\n" "Skipped (unknown method):" "${CNT_SKIPPED}"
...
if ${DRY_RUN}; then
    ...
    log_summary "Servers affected      : ${CNT_HUNG_HOSTS}"
else
...
if [[ -s "${HUNG_HOSTS}" ]]; then
    echo "  Hung log  : ${HUNG_LOG}"
    echo "  Hosts file: ${HUNG_HOSTS}"
    echo
    if ${DRY_RUN}; then
        echo "  To recover, run:"
    else
        echo "  To retry still-hung servers, run:"
    fi
    echo "    ./${SCRIPT_NAME} --hosts ${HUNG_HOSTS}"
else
    # Nothing left hung — clean up the empty files
    rm -f "${HUNG_LOG}" "${HUNG_HOSTS}"
fi
